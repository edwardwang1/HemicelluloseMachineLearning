...
def run_NN(X,Y):
    ...
    saveDF.to_csv("NNRawResultsMonomer.csv", index=False)
...
def run_SVR(X,Y):
    ...
    saveDF = pd.DataFrame(list(zip(master_actual, master_pred)), columns =['Actual', 'Pred'])
    saveDF.to_csv("SVRRawResultsMonomer.csv", index=False)
...
def run_Ridge(X,Y):
    ...
    saveDF = pd.DataFrame(list(zip(master_actual, master_pred)), columns =['Actual', 'Pred'])
    saveDF.to_csv("RidgeRawResultsMonomer.csv", index=False)
...
results.to_csv("MLResultsMonomer.csv")
